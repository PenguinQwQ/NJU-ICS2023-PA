...
// adding some csr instructions
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, I, s->dnpc = isa_raise_intr(100, s->pc)); //ECALL
  //TO Fix, maybe have potential bugs...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, I, 
                                                              if(rd == 0)
                                                              {
                                                                Log("Write %d from reg %llu to csr id : %llu", (word_t)src1, BITS(INSTPAT_INST(s), 19,15), csr_id(INSTPAT_INST(s)));
